// src/Sections/Home/WhyChooseUs/WhyChooseUs.jsx
"use client";
...
const WhyChooseUs = ({ data }) => {
  const { features = [], highlights = [], head = {} } = data || {};
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mx-auto mb-8 max-w-3xl text-center sm:mb-12 md:mb-16">
          <div className="mb-3 inline-flex items-center gap-1.5 rounded-full bg-[#FFD700]/10 px-3 py-1.5 sm:mb-4 sm:gap-2 sm:px-4 sm:py-2">
            {renderIcon("star", "h-4 w-4 text-[#FFD700] sm:h-4.5 sm:w-4.5")}
        ...
        <div className="mb-12 grid grid-cols-1 gap-4 sm:mb-16 sm:grid-cols-2 sm:gap-6 md:gap-8 lg:grid-cols-3">
          {features.map((feature, index) => (
            ...
            >
              ...
              >
                {renderIcon(feature.icon, `h-5 w-5 sm:h-6 sm:w-6 text-black group-hover:text-yellow-500`)}
              </div>
              ...
              <div key={index} className="group text-center">
                <div className="mb-2 flex justify-center sm:mb-3">
                  <div className="rounded-full bg-white/10 p-2.5 transition-colors duration-300 group-hover:bg-[#FFD700] sm:p-3">
                    {renderIcon(item.icon, "h-5 w-5 text-[#FFD700] transition-colors group-hover:text-[#2C4549] sm:h-6 sm:w-6")}
                  </div>
                </div>
        ...
        <div className="mt-12 text-center sm:mt-16">
          <div className="flex flex-col items-center justify-center gap-3 sm:flex-row sm:gap-4">
            <Link
              href={head?.buttons?.bookNow?.link}
            ...
            >
              {head?.buttons?.bookNow?.text}
              {renderIcon("thumbsUp", "h-4 w-4 sm:h-4.5 sm:w-4.5")}
            </Link>
            <Link
              href={head?.buttons?.viewRooms?.text}
            ...
            >
              {head?.buttons?.viewRooms?.text}
              {renderIcon("star", "h-4 w-4 sm:h-4.5 sm:w-4.5")}
            </Link>
          </div>
