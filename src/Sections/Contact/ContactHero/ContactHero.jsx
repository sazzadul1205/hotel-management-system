// src/Sections/Contact/ContactHero/ContactHero.jsx
"use client";
...
const ContactHero = ({ content = {} }) => {
  ...
  const {
    ...
    description = "Have questions about your stay, need assistance with booking, or want to learn more about our services? Our dedicated team is ready to assist you 24/7.",
    backgroundImage = "https://images.unsplash.com/photo-1556742031-c6961e8560b0?ixlib=rb-4.0.3&auto=format&fit=crop&w=2070&q=80",
    contactMethods = [
      { name: "Call Us", icon: "phone", description: "24/7 Support", action: "tel:+15551234567" },
      { name: "Email Us", icon: "email", description: "Reply within 24h", action: "mailto:info@dahotel.com" },
      { name: "Live Chat", icon: "chat", description: "Instant Response", action: "#chat" },
      { name: "WhatsApp", icon: "whatsapp", description: "Message us", action: "https://wa.me/15551234567" },
    ],
    stats = [
      { value: "24/7", label: "Support Available", icon: "clock" },
      { value: "30min", label: "Average Response", icon: "star" },
  ...
  const renderIcon = (iconName, className = "h-4 w-4 sm:h-5 sm:w-5") => {
    ...
    switch (iconName) {
      ...
      case "chat":
        return <MdOutlineHeadsetMic className={iconClass} />;
      case "whatsapp":
  ...
  return (
    <div className="relative min-h-[60vh] w-full overflow-hidden md:min-h-[70vh] lg:min-h-[80vh]">
      ...
      <Image
        src={backgroundImage}
        alt="DA Hotel Contact - We're Here to Help"
        fill
        priority
        ...
        <div className="w-full max-w-5xl animate-fade-in-up">
          ...
          <div className="mb-3 flex items-center justify-center gap-2 md:mb-4">
            <div className="flex h-6 w-6 items-center justify-center rounded-full bg-[#FFD700]/20">
              <MdOutlineEmail className="h-3 w-3 text-[#FFD700] sm:h-3.5 sm:w-3.5" />
            </div>
          ...
          <div className="mb-6 flex flex-wrap justify-center gap-3 sm:mb-8">
            {contactMethods.map((method, index) => (
              <a
                key={index}
                href={method.action}
                className="flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 backdrop-blur-sm transition hover:bg-[#FFD700] hover:text-[#2C4549]"
              >
          ...
          <div className="mt-6 flex items-center justify-center gap-2">
            ...
            <span className="text-xs text-gray-300">Rated #1 for Customer Support</span>
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-4 left-1/2 hidden -translate-x-1/2 transform animate-bounce sm:bottom-6 md:block">
        ...
        <p className="mt-2 hidden text-xs text-gray-300 sm:block">Contact Us</p>
      </div>
