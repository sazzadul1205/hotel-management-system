// src/Sections/Rooms/RoomHero/RoomHero.jsx
"use client";
...
const RoomHero = ({ content = {} }) => {
  // Destructure content with fallbacks
  const {
    title = "Luxurious Rooms & Suites",
    subtitle = "Your Perfect Stay Awaits",
    description = "Experience unparalleled comfort in our beautifully designed rooms and suites. Each space is thoughtfully curated to provide the perfect blend of luxury, comfort, and modern amenities for an unforgettable stay.",
    backgroundImage = "https://images.unsplash.com/photo-1618773928121-c32242e63f39?ixlib=rb-4.0.3&auto=format&fit=crop&w=2070&q=80",
    primaryCta = {
      text: "View Our Rooms",
      link: "#room-list",
    },
    ...
    stats = [
      { value: "150+", label: "Luxury Rooms", icon: "users" },
      { value: "350+", label: "Sq Ft Average", icon: "maximize" },
      { value: "24/7", label: "Room Service", icon: "heart" },
      { value: "4.9", label: "Guest Rating", icon: "award" },
    ],
    roomTypes = [
      { name: "Deluxe", price: "$129", icon: "king" },
      { name: "Executive", price: "$199", icon: "king" },
      { name: "Family Suite", price: "$249", icon: "king" },
      { name: "Presidential", price: "$399", icon: "king" },
    ],
  } = content;
  ...
  const renderIcon = (iconName, className = "h-4 w-4 sm:h-5 sm:w-5") => {
    ...
    switch (iconName) {
      ...
      case "king":
        return <MdOutlineKingBed className={iconClass} />;
      default:
        return <FiHeart className={iconClass} />;
    }
  };
  ...
  return (
    <div className="relative min-h-[60vh] w-full overflow-hidden md:min-h-[70vh] lg:min-h-[80vh]">
      ...
      <Image
        src={backgroundImage}
        alt="DA Hotel Luxury Rooms - Comfort and Elegance"
        fill
        priority
      ...
      <div className="absolute inset-0 bg-linear-to-r from-black/80 via-black/60 to-black/70 md:from-black/70 md:to-black/60">
        ...
        <div className="w-full max-w-5xl animate-fade-in-up">
          ...
          <div className="mb-3 flex items-center justify-center gap-2 md:mb-4">
            <div className="flex h-6 w-6 items-center justify-center rounded-full bg-[#FFD700]/20">
              <MdOutlineKingBed className="h-3 w-3 text-[#FFD700] sm:h-3.5 sm:w-3.5" />
            </div>
          ...
          <h1 className="mb-4 text-3xl font-bold leading-tight tracking-tight sm:text-4xl md:mb-6 md:text-5xl lg:text-6xl xl:text-7xl">
            {title.includes("Luxurious") ? (
              <>
                {title.replace("Luxurious", "")}
          ...
          <div className="mb-6 flex flex-wrap justify-center gap-2 sm:mb-8 sm:gap-3">
            {roomTypes.map((room, index) => (
              ...
              >
                {renderIcon(room.icon, "h-3 w-3 sm:h-3.5 sm:w-3.5")}
                <span className="text-xs font-medium sm:text-sm">
                  {room.name} <span className="font-bold">{room.price}</span>
                </span>
              </div>
            ))}
          </div>
          ...
          <div className="mt-6 flex items-center justify-center gap-2">
            ...
            <span className="text-xs text-gray-300">Rated #1 for Luxury Accommodations</span>
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-4 left-1/2 hidden -translate-x-1/2 transform animate-bounce sm:bottom-6 md:block">
        ...
        <p className="mt-2 hidden text-xs text-gray-300 sm:block">View Rooms</p>
      </div>
